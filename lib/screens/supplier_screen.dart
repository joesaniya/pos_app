...
Color _payColor(PaymentStatus s) {
  switch (s) {
    case PaymentStatus.paid:    return SC.paid;
    case PaymentStatus.pending: return SC.pending;
    case PaymentStatus.overdue: return SC.overdue;
    case PaymentStatus.partial: return SC.partial;
  }
}
Color _payBg(PaymentStatus s) {
  switch (s) {
    case PaymentStatus.paid:    return SC.paidBg;
    case PaymentStatus.pending: return SC.pendingBg;
    case PaymentStatus.overdue: return SC.overdueBg;
    case PaymentStatus.partial: return SC.partialBg;
  }
}
Color _supColor(SupplierStatus s) {
  switch (s) {
    case SupplierStatus.active:      return SC.active;
    case SupplierStatus.inactive:    return SC.inactive;
    case SupplierStatus.blacklisted: return SC.blacklisted;
  }
}
Color _supBg(SupplierStatus s) {
  switch (s) {
    case SupplierStatus.active:      return SC.activeBg;
    case SupplierStatus.inactive:    return SC.inactiveBg;
    case SupplierStatus.blacklisted: return SC.blackBg;
  }
}
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text('pending',
                      style: TextStyle(fontSize: 10, color: SC.amberBright, fontWeight: FontWeight.w500)),
                ],
              ),
            ),
        ],
...
class _AlertBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final expiringDocs = prov.filtered.where((s) => s.hasExpiringDocs || s.hasExpiredDocs).length;
...
class _AlertChip extends StatelessWidget {
  ...
  const _AlertChip({required this.emoji, required this.label, required this.color, required this.bg});
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ListView(
        ...
        children: [
          _MetricCard(label: 'Total Suppliers', value: '${prov.filtered.length}', emoji: '🏭', color: SC.primary),
          _MetricCard(label: 'Pending Amount', value: '₹${_fmt(prov.totalPending)}', emoji: '🕐', color: SC.pending),
          _MetricCard(label: 'Overdue', value: '₹${_fmt(prov.totalOverdue)}', emoji: '🔴', color: SC.overdue),
          _MetricCard(label: 'Active', value: '${prov.activeCount}', emoji: '✅', color: SC.active),
        ],
      ),
    );
  }
...
class _MetricCard extends StatelessWidget {
  ...
  final Color color;
  const _MetricCard({required this.label, required this.value, required this.emoji, required this.color});
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
        Text(value, style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: color, letterSpacing: -0.4)),
        Text(label, style: const TextStyle(fontSize: 10, color: SC.textSec, fontWeight: FontWeight.w500)),
      ]),
    ]),
  );
}
...
class _SearchSortBar extends StatelessWidget {
      ...
      child: Row(children: [
          ...
          child: SizedBox(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: const Icon(Icons.search_rounded, color: SC.textMute, size: 19),
                suffixIcon: ctrl.text.isNotEmpty
                    ? GestureDetector(
                        onTap: () { ctrl.clear(); prov.setSearch(''); },
                        child: const Icon(Icons.close_rounded, size: 16, color: SC.textMute))
                    : null,
                filled: true, fillColor: SC.surface,
                contentPadding: EdgeInsets.zero,
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: SC.border)),
                enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: SC.border)),
                focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: SC.primary, width: 1.5)),
              ),
            ),
          ),
        ),
        ...
        GestureDetector(
          ...
          child: Container(
            height: 44, width: 44,
            decoration: BoxDecoration(
              color: SC.surface, borderRadius: BorderRadius.circular(12),
              border: Border.all(color: SC.border),
            ),
            child: const Icon(Icons.sort_rounded, color: SC.textSec, size: 20),
          ),
        ),
      ]),
    );
  ...
  void _showSort(BuildContext ctx) {
    showModalBottomSheet(
      context: ctx, backgroundColor: SC.surface,
        ...
        return Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Center(child: Container(width: 36, height: 4, margin: const EdgeInsets.only(bottom: 16),
                decoration: BoxDecoration(color: SC.divider, borderRadius: BorderRadius.circular(2)))),
            const Text('Sort By', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: SC.textPri)),
            const SizedBox(height: 12),
            ...opts.entries.map((e) {
              final isSel = prov.sort == e.key;
              return GestureDetector(
                onTap: () { prov.setSort(e.key); Navigator.pop(ctx); },
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: isSel ? SC.primary : Colors.transparent),
                  ),
                  child: Row(children: [
                    Text(e.value.$2, style: const TextStyle(fontSize: 16)),
                    const SizedBox(width: 12),
                    Expanded(child: Text(e.value.$1, style: TextStyle(fontSize: 14,
                      fontWeight: isSel ? FontWeight.w700 : FontWeight.w500,
                      color: isSel ? SC.primary : SC.textPri))),
                    if (isSel) const Icon(Icons.check_circle, color: SC.primary, size: 18),
                  ]),
                ),
              );
            }),
...
class _StatusPills extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const statuses = <SupplierStatus?>[null, SupplierStatus.active, SupplierStatus.inactive, SupplierStatus.blacklisted];
        ...
        return Padding(
          ...
          child: GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: isSel ? (s == null ? SC.primaryLight : _supBg(s)) : Colors.transparent,
...
class _SupplierCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final s = supplier;
    final hasAlert = s.totalOverdue > 0 || s.hasExpiredDocs || s.hasExpiringDocs;
      ...
      child: Container(
        ...
        child: Column(children: [
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Expanded(child: Text(s.name,
                        style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: SC.textPri, letterSpacing: -0.2),
                        maxLines: 1, overflow: TextOverflow.ellipsis)),
                    _StatusBadge(status: s.status),
                  ]),
                  const SizedBox(height: 3),
                  Row(children: [
                    Text(s.category, style: const TextStyle(fontSize: 12, color: SC.textSec)),
                    if (s.city != null) ...[
                      const Text(' · ', style: TextStyle(color: SC.textMute)),
                      Text(s.city!, style: const TextStyle(fontSize: 12, color: SC.textMute)),
                    ],
                  ]),
          ...
          Container(
            ...
            child: Row(children: [
              _CardStat(label: 'Pending', value: s.totalPending > 0 ? '₹${_fmt(s.totalPending)}' : '—',
                  color: s.totalPending > 0 ? SC.pending : SC.textMute),
              _StatDiv(),
              _CardStat(label: 'Overdue', value: s.totalOverdue > 0 ? '₹${_fmt(s.totalOverdue)}' : '—',
                  color: s.totalOverdue > 0 ? SC.overdue : SC.textMute),
              _StatDiv(),
              _CardStat(label: 'Deliveries', value: '${s.deliveries.length}', color: SC.primary),
              _StatDiv(),
              _CardStat(label: 'Tenure', value: s.tenureLabel, color: SC.textSec),
            ]),
          ),
        ]),
      ),
...
class _CardStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Expanded(
    child: Column(children: [
      ...
      Text(label, style: const TextStyle(fontSize: 10, color: SC.textMute, fontWeight: FontWeight.w500)),
    ]),
  );
}
...
class _StatDiv extends StatelessWidget {
  @override
  Widget build(BuildContext context) => Container(width: 1, height: 24, color: SC.divider);
}
  ...
  Widget build(BuildContext context) {
    return Consumer<SupplierProvider>(
      builder: (ctx, prov, _) {
        ...
        final fresh = prov.filtered.where((s) => s.id == _supplier.id).firstOrNull;
...
class _DetailHero extends StatelessWidget {
  ...
  const _DetailHero({required this.supplier, required this.onBack, required this.onEdit});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: const Row(children: [
                ...
                SizedBox(width: 5),
                Text('Edit', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w700)),
              ]),
            ),
          ),
        ]),
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(s.name, style: const TextStyle(
                fontSize: 20, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: -0.4)),
              const SizedBox(height: 4),
              Row(children: [
                Text(s.category, style: TextStyle(fontSize: 12, color: Colors.white.withOpacity(0.65))),
                if (s.city != null) ...[
                  Text(' · ', style: TextStyle(color: Colors.white.withOpacity(0.4))),
                  Text(s.city!, style: TextStyle(fontSize: 12, color: Colors.white.withOpacity(0.65))),
                ],
              ]),
        ...
        // Quick stats
        Row(children: [
          _HeroStat(label: 'Total Business', value: '₹${_fmt(s.totalPurchased)}'),
          _HeroDivider(),
          _HeroStat(label: 'Pending', value: s.totalPending > 0 ? '₹${_fmt(s.totalPending)}' : '—',
              valueColor: s.totalPending > 0 ? SC.amberBright : null),
          _HeroDivider(),
          _HeroStat(label: 'On-time Delivery', value: '${s.deliveryScore.toInt()}%'),
          _HeroDivider(),
  ...
  Widget build(BuildContext context) => Expanded(
    child: Column(children: [
      Text(value, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900,
          color: valueColor ?? Colors.white, letterSpacing: -0.3)),
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        _InfoBlock(items: [
          if (s.gstNumber != null) _InfoItem(icon: '🪪', label: 'GST Number', value: s.gstNumber!),
          if (s.address != null) _InfoItem(icon: '📍', label: 'Address', value: s.address!),
          _InfoItem(icon: '💳', label: 'Credit Limit', value: '₹${_fmt(s.creditLimit)}'),
          _InfoItem(icon: '📅', label: 'Credit Days', value: '${s.creditDays} days'),
          _InfoItem(icon: '🗓️', label: 'Onboarded', value: () {
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: SC.surface, borderRadius: BorderRadius.circular(16),
            border: Border.all(color: SC.border),
          ),
          child: Column(children: [
            _PerfRow('On-time Deliveries', '${s.onTimeDeliveries} / ${s.deliveries.length}',
                '${s.deliveryScore.toInt()}%', SC.active),
            const Divider(height: 20, color: SC.divider),
            _PerfRow('Total Purchased', _fmt(s.totalPurchased), '₹', SC.primary),
            const Divider(height: 20, color: SC.divider),
            _PerfRow('Outstanding Balance', _fmt(s.totalPending), '₹', s.totalPending > 0 ? SC.pending : SC.textMute),
            if (s.totalOverdue > 0) ...[
              const Divider(height: 20, color: SC.divider),
              _PerfRow('Overdue Amount', _fmt(s.totalOverdue), '₹', SC.overdue),
            ],
          ]),
        ),
        ...
        if (s.notes != null) ...[
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: SC.amberLight, borderRadius: BorderRadius.circular(14),
              border: Border.all(color: SC.amber.withOpacity(0.25)),
            ),
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('📝', style: TextStyle(fontSize: 16)),
              const SizedBox(width: 10),
              Expanded(child: Text(s.notes!, style: const TextStyle(fontSize: 13, color: SC.textSec))),
            ]),
          ),
        ],
      ],
...
class _PerfRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
    ...
    children: [
      ...
      Row(children: [
        Text(mainValue, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: color)),
        const SizedBox(width: 3),
        Text(subValue, style: TextStyle(fontSize: 11, color: color.withOpacity(0.7))),
      ]),
    ],
  );
}
    ...
    return Column(children: [
      ...
      if (pendingAmt > 0 || overdueAmt > 0)
        Container(
          ...
          child: Row(children: [
            if (pendingAmt > 0) Expanded(child: _PaySummaryTile(
                label: 'Total Pending', value: '₹${_fmt(pendingAmt)}', color: SC.pending, bg: SC.pendingBg)),
            if (pendingAmt > 0 && overdueAmt > 0) const SizedBox(width: 10),
            if (overdueAmt > 0) Expanded(child: _PaySummaryTile(
                label: 'Overdue', value: '₹${_fmt(overdueAmt)}', color: SC.overdue, bg: SC.overdueBg)),
          ]),
        ),
      ...
      Padding(
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: SC.primaryLight, borderRadius: BorderRadius.circular(12),
              border: Border.all(color: SC.primary.withOpacity(0.3)),
            ),
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.add, color: SC.primary, size: 17),
              SizedBox(width: 6),
              Text('Record Payment', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: SC.primary)),
            ]),
          ),
        ),
      ),
      ...
      Expanded(
        child: supplier.payments.isEmpty
            ? const Center(child: Text('No payment records', style: TextStyle(color: SC.textMute)))
            : ListView.separated(
                padding: const EdgeInsets.fromLTRB(16, 12, 16, 24),
                itemCount: supplier.payments.length,
                separatorBuilder: (_, __) => const SizedBox(height: 8),
                itemBuilder: (_, i) => _PaymentCard(payment: supplier.payments[i]),
              ),
      ),
    ]);
  }
...
class _PaySummaryTile extends StatelessWidget {
  final String label, value;
  final Color color, bg;
  const _PaySummaryTile({required this.label, required this.value, required this.color, required this.bg});
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      Expanded(child: Text(label, style: const TextStyle(fontSize: 11, color: SC.textSec))),
...
class _PaymentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
        color: SC.surface, borderRadius: BorderRadius.circular(14),
        border: Border.all(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Expanded(child: Text(p.description,
                      style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: SC.textPri),
                      maxLines: 1, overflow: TextOverflow.ellipsis)),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(20)),
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      Text(p.status.emoji, style: const TextStyle(fontSize: 10)),
                ...
                const SizedBox(height: 6),
                Row(children: [
                  Text(p.mode.emoji, style: const TextStyle(fontSize: 12)),
                  const SizedBox(width: 4),
                  Text(p.mode.label, style: const TextStyle(fontSize: 11, color: SC.textSec)),
                  const SizedBox(width: 8),
                  Text('·', style: const TextStyle(color: SC.textMute)),
                  const SizedBox(width: 8),
                  Text(p.dateLabel, style: const TextStyle(fontSize: 11, color: SC.textMute)),
                  if (p.invoiceRef != null) ...[
                    const SizedBox(width: 8),
                    Text('·', style: const TextStyle(color: SC.textMute)),
                    const SizedBox(width: 8),
                    Text(p.invoiceRef!, style: const TextStyle(fontSize: 11, color: SC.primary, fontWeight: FontWeight.w600)),
                  ],
                ]),
                const SizedBox(height: 8),
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('₹${_fmt(p.amount)}',
                        style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: SC.textPri)),
                    if (p.status == PaymentStatus.partial && p.paidAmount != null)
                      Text('Paid ₹${_fmt(p.paidAmount!)} · Balance ₹${_fmt(p.outstanding)}',
                          style: TextStyle(fontSize: 11, color: SC.partial)),
                  ]),
                  if (p.dueDate != null)
                    Container(
    ...
    return Column(children: [
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              SizedBox(width: 6),
              Text('Upload Document', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: SC.primary)),
            ]),
          ),
        ),
      ),
      ...
      Expanded(
        child: supplier.documents.isEmpty
            ? const Center(child: Text('No documents uploaded', style: TextStyle(color: SC.textMute)))
            : ListView.separated(
                padding: const EdgeInsets.fromLTRB(16, 12, 16, 24),
                itemCount: supplier.documents.length,
                separatorBuilder: (_, __) => const SizedBox(height: 8),
                itemBuilder: (_, i) => _DocumentCard(doc: supplier.documents[i]),
              ),
      ),
    ]);
  }
...
class _DocumentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final borderColor = isExp ? SC.overdue : isSoon ? SC.amber : SC.border;

    return Container(
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(
        color: SC.surface, borderRadius: BorderRadius.circular(14),
        border: Border.all(color: borderColor.withOpacity(isExp || isSoon ? 0.5 : 1), width: isExp || isSoon ? 1.5 : 1),
      ...
      child: Row(children: [
        Container(
          width: 44, height: 44,
          decoration: BoxDecoration(
            color: isExp ? SC.overdueBg : isSoon ? SC.amberLight : SC.primaryLight,
            borderRadius: BorderRadius.circular(12),
          ),
          alignment: Alignment.center,
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(doc.title,
                style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: SC.textPri),
                maxLines: 1, overflow: TextOverflow.ellipsis),
            const SizedBox(height: 3),
            Row(children: [
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
                decoration: BoxDecoration(color: SC.primaryLight, borderRadius: BorderRadius.circular(6)),
                child: Text(doc.type.label,
                    style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: SC.primary)),
              ),
              const SizedBox(width: 8),
              Text(_uploadDate(doc.uploadedOn),
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: isExp ? SC.overdueBg : isSoon ? SC.amberLight : SC.surfaceAlt,
            borderRadius: BorderRadius.circular(8),
          ),
          child: Text(doc.expiryLabel,
              style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700,
                  color: isExp ? SC.overdue : isSoon ? SC.amber : SC.textMute)),
        ),
      ]),
    );
  }
...
class _HistoryTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (deliveries.isEmpty) {
      return const Center(child: Text('No delivery history', style: TextStyle(color: SC.textMute)));
    }
          ...
          child: Row(children: [
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 4),
                Text('$onTime/$total on-time',
                    style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Colors.white)),
              ]),
            ),
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text('${supplier.deliveryScore.toInt()}%',
                  style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: SC.amberBright)),
              const Text('score', style: TextStyle(fontSize: 11, color: Colors.white54)),
            ]),
          ]),
        ),
        const SizedBox(height: 14),
        ...
        Column(children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: d.onTime ? SC.active : SC.overdue, width: 1.5),
            ),
            alignment: Alignment.center,
            child: Icon(d.onTime ? Icons.check_rounded : Icons.warning_amber_rounded,
                color: d.onTime ? SC.active : SC.overdue, size: 15),
          ),
          if (!isLast) Expanded(child: Container(width: 1.5, color: SC.divider)),
        ]),
        const SizedBox(width: 12),
        // Card
        Expanded(
          child: Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text(d.dateLabel,
                    style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: SC.textPri))),
                Text('₹${_fmt(d.totalValue)}',
                    style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w900, color: SC.primary)),
              ]),
              const SizedBox(height: 6),
              Wrap(spacing: 6, runSpacing: 4, children: d.items.map((item) =>
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: BoxDecoration(color: SC.surfaceAlt, borderRadius: BorderRadius.circular(6),
                      border: Border.all(color: SC.border)),
                  child: Text(item, style: const TextStyle(fontSize: 11, color: SC.textSec)),
                )).toList()),
              if (d.note != null) ...[
                const SizedBox(height: 6),
                Row(children: [
                  ...
                  Expanded(child: Text(d.note!,
                      style: const TextStyle(fontSize: 11, color: SC.textSec, fontStyle: FontStyle.italic))),
                ]),
              ],
            ]),
          ),
...
class _AddPaymentSheetState extends State<_AddPaymentSheet> {
  ...
  @override
  void dispose() {
    _amtCtrl.dispose(); _descCtrl.dispose(); _refCtrl.dispose();
    super.dispose();
  }
    ...
    return Container(
      ...
      child: Form(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          _Handle(), _SheetTop(emoji: '💳', title: 'Record Payment', subtitle: 'Add a payment entry', color: SC.primary),
          Flexible(
            child: SingleChildScrollView(
              padding: const EdgeInsets.fromLTRB(20, 14, 20, 28),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                _FieldLabel('Amount (₹) *'),
                _TField(ctrl: _amtCtrl, hint: '0.00', type: TextInputType.number,
                    validator: (v) => (v == null || v.isEmpty) ? 'Required' : null),
                const SizedBox(height: 12),
                _FieldLabel('Description *'),
                _TField(ctrl: _descCtrl, hint: 'What is this payment for?',
                    validator: (v) => (v == null || v.isEmpty) ? 'Required' : null),
                const SizedBox(height: 12),
                    ...
                    return GestureDetector(
                      ...
                      child: AnimatedContainer(duration: const Duration(milliseconds: 140),
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(10),
                          border: Border.all(color: isSel ? _payColor(s) : SC.border, width: isSel ? 1.5 : 1),
                        ),
                        child: Row(mainAxisSize: MainAxisSize.min, children: [
                          Text(s.emoji, style: const TextStyle(fontSize: 13)),
                          const SizedBox(width: 5),
                          Text(s.label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700,
                    ...
                    return GestureDetector(
                      ...
                      child: AnimatedContainer(duration: const Duration(milliseconds: 140),
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(10),
                          border: Border.all(color: isSel ? SC.primary : SC.border, width: isSel ? 1.5 : 1),
                        ),
                        child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                SizedBox(width: double.infinity,
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: SC.primary, foregroundColor: Colors.white,
                    ...
                    child: _loading
                        ...
                        : const Text('Save Payment', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800)),
                  ),
                ),
              ]),
            ),
...
class _AddDocumentSheetState extends State<_AddDocumentSheet> {
  ...
  Future<void> _submit() async {
    ...
    final doc = SupplierDocument(
      ...
      uploadedOn: DateTime.now(), expiryDate: _expiry,
    );
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom),
      decoration: const BoxDecoration(color: SC.surface, borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        _Handle(), _SheetTop(emoji: '📎', title: 'Upload Document', subtitle: 'Add a new document record', color: SC.primary),
        Flexible(
          child: SingleChildScrollView(
            padding: const EdgeInsets.fromLTRB(20, 14, 20, 28),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  return GestureDetector(
                    ...
                    child: AnimatedContainer(duration: const Duration(milliseconds: 140),
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        const SizedBox(width: 5),
                        Text(t.label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700,
                            color: isSel ? SC.primary : SC.textSec)),
                      ]),
                    ),
                  );
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: SC.surfaceAlt, borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: SC.border),
                  ),
                  child: Row(children: [
                    const Text('📅', style: TextStyle(fontSize: 16)),
                    const SizedBox(width: 8),
                    Text(
                      _expiry == null ? 'Set expiry date (optional)' : () {
                        const m = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
              ...
              SizedBox(width: double.infinity,
                child: ElevatedButton(
                  ...
                  child: const Text('Save Document', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800)),
                ),
              ),
            ]),
          ),
...
class _AddEditSupplierSheetState extends State<_AddEditSupplierSheet> {
  final _formKey   = GlobalKey<FormState>();
  late TextEditingController _nameCtrl, _cityCtrl, _gstCtrl, _addrCtrl, _notesCtrl;
  ...
  static const _categories = ['Grains & Pulses','Dairy','Vegetables','Oils','Spices','Herbs','Beverages','Packaging','Other'];
  ...
  @override
  void dispose() {
    for (final c in [_nameCtrl, _cityCtrl, _gstCtrl, _addrCtrl, _notesCtrl]) c.dispose();
    super.dispose();
  }
  ...
  Future<void> _submit() async {
    ...
    final sup = Supplier(
      ...
      creditLimit: _creditLimit, creditDays: _creditDays,
    ...
    return DraggableScrollableSheet(
      ...
      builder: (_, ctrl) => Container(
        ...
        child: Form(
          key: _formKey,
          child: Column(children: [
            _Handle(), _SheetTop(emoji: isEdit ? '✏️' : '➕',
                title: isEdit ? 'Edit Supplier' : 'Add Supplier',
                subtitle: isEdit ? 'Update supplier details' : 'Register a new supplier',
                color: SC.primary),
            Expanded(
              child: ListView(controller: ctrl, padding: const EdgeInsets.fromLTRB(20, 14, 20, 28), children: [
                // Emoji
                _FieldLabel('Icon'),
                const SizedBox(height: 8),
                SizedBox(height: 50,
                        ...
                        child: GestureDetector(
                          ...
                          child: AnimatedContainer(duration: const Duration(milliseconds: 140),
                            ...
                            decoration: BoxDecoration(
                              color: isSel ? SC.primaryLight : SC.surfaceAlt,
                              borderRadius: BorderRadius.circular(12),
                              border: Border.all(color: isSel ? SC.primary : SC.border, width: isSel ? 2 : 1),
                            ),
                ...
                _TField(ctrl: _nameCtrl, hint: 'e.g. Sri Annapoorna Traders',
                    validator: (v) => (v == null || v.isEmpty) ? 'Required' : null),
                const SizedBox(height: 12),
                  ...
                  children: _categories.map((c) {
                    ...
                    return GestureDetector(
                      ...
                      child: AnimatedContainer(duration: const Duration(milliseconds: 140),
                        ...
                        child: Text(c, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700,
                            color: isSel ? SC.primary : SC.textSec)),
                      ),
                    );
                  }).toList(),
                        ...
                        children: [7, 14, 21, 30, 45, 60].map((d) {
                          ...
                          return GestureDetector(
                            ...
                            child: AnimatedContainer(duration: const Duration(milliseconds: 130),
                              ...
                              child: Text('$d days', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                                  color: isSel ? SC.primary : SC.textSec)),
                            ),
                          );
                        }).toList(),
                ...
                _FieldLabel('Status'),
                const SizedBox(height: 8),
                Row(children: SupplierStatus.values.map((s) {
                  final isSel = _status == s;
                  return Padding(padding: const EdgeInsets.only(right: 8),
                    child: GestureDetector(
                      onTap: () => setState(() => _status = s),
                      child: AnimatedContainer(duration: const Duration(milliseconds: 130),
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(10),
                          border: Border.all(color: isSel ? _supColor(s) : SC.border, width: isSel ? 1.5 : 1),
                        ),
                        child: Text(s.label, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700,
                ...
                _FieldLabel('Notes'),
                _TField(ctrl: _notesCtrl, hint: 'Any special instructions or remarks...'),
                  ...
                  if (isEdit) ...[
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: SC.overdueBg, borderRadius: BorderRadius.circular(14),
                          border: Border.all(color: SC.overdue.withOpacity(0.3)),
                        ),
                        child: const Icon(Icons.delete_outline, color: SC.overdue, size: 20),
                      ),
                    ),
                    const SizedBox(width: 10),
                  ],
  ...
  void _confirmDelete(BuildContext ctx) {
    ...
    showDialog(context: ctx, builder: (_) => AlertDialog(
      ...
      title: Text('Delete ${widget.editSupplier!.name}?',
          style: const TextStyle(fontWeight: FontWeight.w800, color: SC.textPri)),
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () {
            ...
            Navigator.pop(ctx); Navigator.pop(ctx); Navigator.pop(ctx);
          },
          style: ElevatedButton.styleFrom(backgroundColor: SC.overdue, foregroundColor: Colors.white,
...
class _ContactCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    padding: const EdgeInsets.all(14),
    decoration: BoxDecoration(color: SC.surface, borderRadius: BorderRadius.circular(14),
        border: Border.all(color: SC.border),
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 6)]),
    child: Row(children: [
      Container(width: 40, height: 40,
        decoration: BoxDecoration(color: SC.primaryLight, shape: BoxShape.circle),
        alignment: Alignment.center,
        child: Text(contact.name[0].toUpperCase(),
            style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: SC.primary))),
      const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(contact.name, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: SC.textPri)),
        Text(contact.role, style: const TextStyle(fontSize: 11, color: SC.textMute)),
      ])),
      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
        Text(contact.phone, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: SC.primary)),
        if (contact.email != null)
          Text(contact.email!, style: const TextStyle(fontSize: 11, color: SC.textMute)),
      ]),
    ]),
  );
}
    ...
    child: Column(children: items.asMap().entries.map((e) => Column(children: [
      Padding(padding: const EdgeInsets.fromLTRB(14, 12, 14, 12),
        child: Row(children: [
          ...
          const SizedBox(width: 10),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(e.value.label, style: const TextStyle(fontSize: 10, color: SC.textMute, fontWeight: FontWeight.w500)),
            Text(e.value.value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: SC.textPri)),
          ])),
        ])),
      if (e.key < items.length - 1) const Divider(height: 1, indent: 16, endIndent: 16, color: SC.divider),
    ])).toList()),
  );
}
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Text(text.toUpperCase(), style: const TextStyle(
        fontSize: 10, fontWeight: FontWeight.w800, color: SC.textMute, letterSpacing: 1.4)),
  );
}
...
class _SheetTop extends StatelessWidget {
  ...
  final Color color;
  const _SheetTop({required this.emoji, required this.title, required this.subtitle, required this.color});
  ...
  Widget build(BuildContext context) => Column(children: [
    Padding(padding: const EdgeInsets.fromLTRB(20, 8, 20, 16), child: Row(children: [
      ...
      const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: SC.textPri, letterSpacing: -0.3)),
        Text(subtitle, style: const TextStyle(fontSize: 12, color: SC.textSec)),
      ])),
    ])),
    const Divider(height: 1, color: SC.divider),
  ]);
...
class _FieldLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Text(text, style: const TextStyle(
      fontSize: 12, fontWeight: FontWeight.w700, color: SC.textSec, letterSpacing: 0.3));
}
...
class _TField extends StatelessWidget {
  ...
  const _TField({required this.ctrl, required this.hint,
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: TextFormField(
      ...
      style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: SC.textPri),
      decoration: InputDecoration(
        hintText: hint, hintStyle: const TextStyle(color: SC.textMute, fontSize: 13),
        filled: true, fillColor: SC.surfaceAlt,
        contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
        border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: SC.border)),
        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: SC.border)),
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: SC.primary, width: 1.5)),
        errorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: SC.overdue)),
      ),
    ),
  );
}
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const Text('No suppliers found', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: SC.textPri)),
      const SizedBox(height: 5),
      const Text('Add your first supplier', style: TextStyle(fontSize: 13, color: SC.textSec)),
    ]),
  );
}
...
class _AddFAB extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        color: SC.primary, borderRadius: BorderRadius.circular(18),
        boxShadow: [BoxShadow(color: SC.primary.withOpacity(0.38), blurRadius: 18, offset: const Offset(0, 8))],
      ),
      child: const Row(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.add_rounded, color: Colors.white, size: 20),
        SizedBox(width: 8),
        Text('Add Supplier', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w800)),
      ]),
    ),
  );
}
