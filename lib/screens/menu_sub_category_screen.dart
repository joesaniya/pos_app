// lib/screens/menu_sub_category_screen.dart
//
// ✅ UPDATED: FAB and item options now route to AddMenuItemWithRecipeScreen
//    instead of AddMenuItemScreen — giving full recipe + inventory integration.
...
// ── CHANGED: import the new unified screen instead of AddMenuItemScreen ──────
...
const Set<String> _editableRoles = {'admin', 'system', 'owner', 'manager'};
...
class _MenuSubcategoryScreenState extends State<MenuSubcategoryScreen> {
  ...
  // ✅ KEY CHANGE: we now push AddMenuItemWithRecipeScreen everywhere.
  //    InventoryProvider is already registered globally in common_provider.dart
  //    so no MultiProvider wrapper is needed here.
  void _openAddItem(BuildContext ctx) {
    Navigator.push(
      ctx,
      MaterialPageRoute(
        builder: (_) =>
            AddMenuItemWithRecipeScreen(category: widget.supabaseCategory!),
      ),
    );
  }

  void _openEditItem(BuildContext ctx, SupabaseMenuItem item) {
    Navigator.push(
      ctx,
      MaterialPageRoute(
                          ...
                          : RefreshIndicator(
                              ...
                              child: GridView.builder(
                                ...
                                itemBuilder: (_, idx) => _SupabaseMenuItemCard(
                                  ...
                                  onEdit: () => _openEditItem(ctx, items[idx]),
                                ),
                              ),
                            ),
                    ),
...
// ═════════════════════════════════════════════════════════════════════════════
//  MENU ITEM CARD  ← onEdit callback added
// ═════════════════════════════════════════════════════════════════════════════
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        ...
        PageRouteBuilder(
          pageBuilder: (_, a, __) =>
              MenuItemDetailScreen(item: item, category: category),
...
// ═════════════════════════════════════════════════════════════════════════════
//  ITEM OPTIONS SHEET  ← onEdit added, routes to AddMenuItemWithRecipeScreen
// ═════════════════════════════════════════════════════════════════════════════
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // ── Edit Item → opens AddMenuItemWithRecipeScreen ────────────
          _OptionTile(
            icon: Icons.edit_outlined,
            label: 'Edit Item & Recipe',
            sublabel: 'Update details, ingredients & inventory',
            color: primaryColor,
            onTap: () {
              final navigator = Navigator.of(context);
              navigator.pop();
              // ✅ Call the onEdit callback which pushes
              //    AddMenuItemWithRecipeScreen with the existing item
              onEdit();
            },
          ),
          ...
          _OptionTile(
            ...
            label: 'Delete Item',
            sublabel: 'Permanently remove from menu',
            color: Colors.red,
            onTap: () async {
              final navigator = Navigator.of(context);
              final prov = provider;
