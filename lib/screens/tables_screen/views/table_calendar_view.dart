...
class _ReservationDetailSheet extends StatelessWidget {
  ...
  // ── Opens the standard ReservationSheet in edit mode ─────────────────────
  // Times are locked inside ReservationSheet when existing != null.
  // We pass widget.existing!.reservedFor and checkOut directly so they
  // are preserved and the UI shows the correct locked values.
  void _openEditSheet(BuildContext context) {
    log('calendar detail: open edit sheet for reservation ${item.id}');

    // Find the table object by tableNumber so we have its UUID
    ...
    if (table == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Table not found — try refreshing.'),
          backgroundColor: TC.occupied,
        ),
      );
      return;
    ...
    // Build a Reservation from the history item so ReservationSheet
    // receives existing != null → triggers edit/locked mode.
    final existing = Reservation(
      id: item.id,
      customerName: item.customerName,
      phone: item.phone,
      guestCount: item.guestCount,
      reservedFor: item.reservedFor, // 🔒 locked in ReservationSheet
      checkOut: item.checkOut, // 🔒 locked in ReservationSheet
      notes: item.notes,
    ...
    showModalBottomSheet(
      ...
      builder: (_) => ChangeNotifierProvider.value(
        ...
        child: ReservationSheet(
          ...
          provider: prov,
          existing:
              existing, // ← passes existing → isEdit = true → times locked
        ),
      ),
    );
  }
