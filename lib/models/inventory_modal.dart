...
//  Returns true only when [id] is a well-formed UUID.
//  Sentinel strings like "other", "unknown", empty, etc. return false
//  so they are serialised as NULL instead of being sent to Postgres.
// ─────────────────────────────────────────────────────────────────────────────
...
class InventoryItem {
  ...
  final double costPerUnit;
  final String supplier; // display name
  final String? supplierId; // FK to suppliers table — may be null
  final DateTime lastUpdated;
  final List<StockTransaction> transactions;
  final String? notes;
  ...
  /// True only when supplierId is a real UUID (not a sentinel / empty string).
  bool get hasLinkedSupplier => _isValidUuid(supplierId);
  ...
  /// Serialises the item for Supabase INSERT / UPDATE.
  /// supplier_id is sent as NULL whenever it is not a valid UUID —
  /// this prevents the "invalid input syntax for type uuid" error that occurs
  /// when sentinel strings like "other" are passed to a UUID column.
  Map<String, dynamic> toJson(String businessId) => {
    'business_id': businessId,
    'name': name,
    'category': category,
    ...
    'cost_per_unit': costPerUnit,
    'supplier_name': supplier,
    'supplier_id': _isValidUuid(supplierId) ? supplierId : null,
    'last_updated': lastUpdated.toIso8601String(),
    'notes': notes,
    'is_active': true,
  };
