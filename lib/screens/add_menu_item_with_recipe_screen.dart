...
class RecipeIngredientEntry {
  ...
  double get qtyInBaseUnit =>
      _toBaseUnit(requiredQty, selectedUnit, item.unit);

  bool get isValid =>
      error == null &&
      requiredQty > 0 &&
      qtyInBaseUnit <= item.currentStock;
...
class _AddMenuItemWithRecipeScreenState
  ...
  void dispose() {
    ...
    for (final c in [
      ...
      _prepTimeCtrl, _servingCtrl,
      _caloriesCtrl, _proteinCtrl, _carbsCtrl, _fatCtrl,
      _fiberCtrl, _sodiumCtrl, _sugarCtrl,
      _recipeNotesCtrl, _ingredSearchCtrl,
    ]) { c.dispose(); }
    super.dispose();
  }
  ...
  Future<void> _loadInventory() async {
    ...
    try {
      ...
      if (mounted) setState(() { _availableInventory = items; _loadingInventory = false; });
    } catch (_) {
  ...
  List<InventoryItem> get _filteredInventory {
    ...
    return _availableInventory.where((i) =>
  ...
  void _addIngredient(InventoryItem item) {
    ...
    setState(() => _recipeIngredients.add(
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final discount = _discountCtrl.text.trim().isNotEmpty
          ? double.tryParse(_discountCtrl.text.trim()) : null;
      ...
      if (_isEdit) {
        ...
        await menu.updateItem(
          id: menuItemId, categoryId: widget.category.id,
          updates: {
            ...
            'serving_size':     _servingCtrl.text.trim().isEmpty ? null : _servingCtrl.text.trim(),
      ...
      if (_hasRecipe && _recipeIngredients.isNotEmpty &&
          menuItemId != null && menuItemId.isNotEmpty) {
  ...
  Future<void> _saveRecipe(
      String menuItemId, SupabaseMenuProvider menu, List<String> allergenList) async {
    ...
    await Supabase.instance.client.from('recipes').upsert({
      ...
      'menu_item_id':             menuItemId,           // NEW — links to menu item
      ...
      'serving_size':             _servingCtrl.text.trim().isEmpty
                                      ? null : _servingCtrl.text.trim(),
      'is_active':                true,
      'is_featured':              _isFeatured,
      'category':                 widget.category.name,
      'notes':                    _recipeNotesCtrl.text.trim().isEmpty
                                      ? null : _recipeNotesCtrl.text.trim(),
      ...
      child: Row(children: [
        GestureDetector(
          ...
          child: Container(
            ...
            child: const Icon(Icons.arrow_back_ios_new_rounded, size: 16, color: _C.textPri),
          ),
        ),
        ...
        Expanded(child: Column(
          ...
          children: [
            ...
            Row(children: [
              Text(widget.category.icon, style: const TextStyle(fontSize: 12)),
              const SizedBox(width: 4),
              Text(widget.category.name, style: const TextStyle(
                  fontSize: 12, color: _C.textSec, fontWeight: FontWeight.w500)),
            ]),
          ],
        )),
        Container(
          ...
          child: Text(
            ...
            style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: _C.brand),
          ),
        ),
      ]),
    );
  ...
  Widget _buildStepBar() {
    ...
    const icons  = [Icons.restaurant_menu_rounded, Icons.monitor_heart_outlined, Icons.science_outlined];
          ...
          return Expanded(child: Row(children: [
            ...
            AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                color: isDone ? _C.stepDone : isActive ? color : _C.stepIdle,
                border: Border.all(
                    color: isDone ? _C.stepDone : isActive ? color : _C.stepIdle,
                    width: isActive ? 2 : 1),
                boxShadow: isActive ? [BoxShadow(
                    color: color.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 3))] : [],
              ),
              child: Center(child: isDone
                  ? const Icon(Icons.check_rounded, color: Colors.white, size: 16)
                  : Icon(icons[i],
                      color: isActive ? Colors.white : _C.textMut, size: 15)),
            ),
              ...
              children: [
                Text('Step ${i + 1}',
                    style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700,
                        color: isDone ? _C.stepDone : isActive ? color : _C.textMut,
                        letterSpacing: 0.8)),
                Text(labels[i],
            ...
            if (i < _totalSteps - 1)
              AnimatedContainer(
                ...
                width: 14, height: 2, margin: const EdgeInsets.symmetric(horizontal: 4),
                decoration: BoxDecoration(
    ...
    return Form(
      ...
      child: ListView(
        ...
        children: [
          ...
          // Veg toggle
          _VegToggleWidget(isVeg: _isVeg, onChanged: (v) => setState(() => _isVeg = v)),
          const SizedBox(height: 16),
          ...
          _SectionCard(
            ...
            child: Column(children: [
              _lbl('Item Name *'), const SizedBox(height: 6),
              _tf(ctrl: _nameCtrl, hint: 'e.g. Chicken Biryani',
                  icon: Icons.fastfood_outlined,
                  capitalization: TextCapitalization.words,
                  validator: (v) => v!.trim().isEmpty ? 'Required' : null),
              const SizedBox(height: 12),
              _lbl('Description'), const SizedBox(height: 6),
              _tf(ctrl: _descCtrl, hint: 'Short description of this dish...',
                  icon: Icons.notes_rounded, maxLines: 2),
              const SizedBox(height: 12),
              _lbl('Serving Size (optional)'), const SizedBox(height: 6),
              _tf(ctrl: _servingCtrl, hint: 'e.g. 1 plate (300g)',
          ...
          _SectionCard(
            ...
            child: Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                _lbl('Price (₹) *'), const SizedBox(height: 6),
                _tf(ctrl: _priceCtrl, hint: '0.00',
                    icon: Icons.currency_rupee_rounded,
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                _lbl('Discount (₹)'), const SizedBox(height: 6),
                _tf(ctrl: _discountCtrl, hint: 'Optional',
          ...
          _SectionCard(
            ...
            child: Column(children: [
              Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  _lbl('Prep Time (min)'), const SizedBox(height: 6),
                  _tf(ctrl: _prepTimeCtrl, hint: '15', icon: Icons.timer_outlined,
                      keyboardType: TextInputType.number,
                      inputFormatters: [FilteringTextInputFormatter.digitsOnly]),
                ])),
              ]),
              const SizedBox(height: 12),
              _lbl('Rating'), const SizedBox(height: 8),
              _RatingSliderWidget(value: _rating, onChanged: (v) => setState(() => _rating = v)),
            ]),
          ),
          const SizedBox(height: 14),
          ...
          _SectionCard(
            ...
            child: Wrap(spacing: 10, runSpacing: 10, children: [
              _FlagChip(emoji: '✅', label: 'Available',    active: _isAvailable,  activeColor: _C.success,           onTap: () => setState(() => _isAvailable  = !_isAvailable)),
              _FlagChip(emoji: '🔥', label: 'Bestseller',  active: _isBestSeller, activeColor: const Color(0xFFEA580C), onTap: () => setState(() => _isBestSeller = !_isBestSeller)),
              _FlagChip(emoji: '⭐', label: 'Featured',    active: _isFeatured,   activeColor: const Color(0xFFCA8A04), onTap: () => setState(() => _isFeatured   = !_isFeatured)),
              _FlagChip(emoji: '🆕', label: 'New Arrival', active: _isNewArrival, activeColor: const Color(0xFF2563EB), onTap: () => setState(() => _isNewArrival  = !_isNewArrival)),
              _FlagChip(emoji: '🌶️', label: 'Spicy',       active: _isSpicy,      activeColor: _C.danger,             onTap: () => setState(() => _isSpicy       = !_isSpicy)),
            ]),
          ),
        ],
      ),
    ...
    return ListView(
      ...
      children: [
        ...
        Container(
          ...
          child: Row(children: [
            ...
            const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('All fields optional — per serving values recommended',
        ...
        _SectionCard(
          ...
          title: 'Energy',
          child: Column(children: [
            _lbl('Calories (kcal)'), const SizedBox(height: 6),
            _tf(ctrl: _caloriesCtrl, hint: 'e.g. 450',
                icon: Icons.local_fire_department_outlined,
                keyboardType: TextInputType.number,
                inputFormatters: [FilteringTextInputFormatter.digitsOnly]),
          ]),
        ...
        _SectionCard(
          ...
          child: Column(children: [
            Row(children: [
              Expanded(child: _NutritionField(ctrl: _proteinCtrl,
                  label: 'Protein',  unit: 'g', emoji: '🥩',
                  color: const Color(0xFF3B82F6))),
              const SizedBox(width: 10),
              Expanded(child: _NutritionField(ctrl: _carbsCtrl,
                  label: 'Carbs',    unit: 'g', emoji: '🍞',
                  color: const Color(0xFFF59E0B))),
            ]),
            const SizedBox(height: 10),
            Row(children: [
              Expanded(child: _NutritionField(ctrl: _fatCtrl,
                  label: 'Fat',     unit: 'g', emoji: '🫙',
                  color: const Color(0xFFEF4444))),
              const SizedBox(width: 10),
              Expanded(child: _NutritionField(ctrl: _fiberCtrl,
                  label: 'Fiber',   unit: 'g', emoji: '🌾',
                  color: const Color(0xFF10B981))),
            ]),
            const SizedBox(height: 10),
            Row(children: [
              Expanded(child: _NutritionField(ctrl: _sodiumCtrl,
                  label: 'Sodium',  unit: 'mg', emoji: '🧂',
                  color: const Color(0xFF8B5CF6))),
              const SizedBox(width: 10),
              Expanded(child: _NutritionField(ctrl: _sugarCtrl,
                  label: 'Sugar',   unit: 'g', emoji: '🍬',
          ...
          child: Column(children: [
            ...
            if (_selectedAllergenIds.isNotEmpty) ...[
              ...
              Container(
                ...
                child: Row(children: [
                  const Icon(Icons.warning_amber_rounded, color: _C.danger, size: 16),
                  const SizedBox(width: 8),
                  Expanded(child: Text(
                    'Contains: ${_selectedAllergenIds.map((id) => _kAllergens.firstWhere((a) => a['id'] == id)['label']).join(', ')}',
                    style: const TextStyle(fontSize: 12, color: _C.danger, fontWeight: FontWeight.w600),
                  )),
                ]),
              ),
            ],
    ...
    return ListView(
      ...
      children: [
        ...
        if (_hasRecipe) ...[
          // Recipe notes
          _SectionCard(
            color: _C.teal, icon: Icons.notes_rounded,
            title: 'Chef Notes (optional)',
            child: Column(children: [
              _tf(ctrl: _recipeNotesCtrl,
                  hint: 'Special instructions, cooking tips, temperature, timing...',
          ...
          if (_recipeIngredients.isNotEmpty) ...[
            ...
            const SizedBox(height: 10),
            ..._recipeIngredients.asMap().entries.map((e) =>
              _IngredientTileWidget(
        ...
        if (!_hasRecipe)
          _InfoBox(
            emoji: '💡',
            text: 'You can add a recipe later. The menu item will be saved without one.',
          ),
      ],
    );
  }
  ...
  Widget _buildBottomBar() {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        if (_saveError != null) ...[
          _ErrorBanner(msg: _saveError!, onDismiss: () => setState(() => _saveError = null)),
          const SizedBox(height: 10),
        ],
        Row(children: [
          if (_currentStep > 0) ...[
          ...
          Expanded(child: isLast
              ? _PrimaryButton(
                  label: _isEdit ? 'Save Changes' : 'Create Item & Recipe',
  ...
  Widget _lbl(String t) => Text(t, style: const TextStyle(
      ...
      color: _C.textSec, letterSpacing: 0.2));
  ...
  Widget _tf({
    ...
    style: const TextStyle(fontSize: 14, color: _C.textPri, fontWeight: FontWeight.w500),
      ...
      Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (subtitle != null)
              Text(subtitle!, style: const TextStyle(fontSize: 11, color: _C.textSec)),
          ])),
        ]),
      ),
...
class _NutritionField extends StatelessWidget {
  ...
  const _NutritionField({
    ...
    required this.unit, required this.emoji, required this.color,
  });
...
class _NutritionPreviewBar extends StatelessWidget {
  ...
  const _NutritionPreviewBar({this.calories, this.protein, this.carbs, this.fat});
    ...
    return Container(
      ...
      child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          _MacroChip(label: 'Calories', value: calories?.toString() ?? '—', unit: 'kcal', color: const Color(0xFFEA580C)),
          _MacroChip(label: 'Protein',  value: protein != null  ? '${protein!.toStringAsFixed(1)}' : '—', unit: 'g', color: const Color(0xFF3B82F6)),
          _MacroChip(label: 'Carbs',    value: carbs != null    ? '${carbs!.toStringAsFixed(1)}' : '—', unit: 'g', color: const Color(0xFFF59E0B)),
          _MacroChip(label: 'Fat',      value: fat != null      ? '${fat!.toStringAsFixed(1)}' : '—', unit: 'g', color: const Color(0xFFEF4444)),
        ]),
        if (total > 0) ...[
          const SizedBox(height: 12),
          ClipRRect(
            ...
            child: Row(children: [
              if ((protein ?? 0) > 0)
                Expanded(flex: ((protein! / total) * 100).round(),
                    child: Container(height: 6, color: const Color(0xFF3B82F6))),
              if ((carbs ?? 0) > 0)
                Expanded(flex: ((carbs! / total) * 100).round(),
                    child: Container(height: 6, color: const Color(0xFFF59E0B))),
              if ((fat ?? 0) > 0)
                Expanded(flex: ((fat! / total) * 100).round(),
...
class _IngredientTileWidget extends StatefulWidget {
  ...
  const _IngredientTileWidget({
    ...
    required this.onRemove, required this.onQtyChanged,
    required this.onUnitChanged, required this.fmtQty,
  });
...
class _IngredientTileWidgetState extends State<_IngredientTileWidget> {
  ...
  void initState() {
    ...
    _qtyCtrl = TextEditingController(
      text: widget.entry.requiredQty > 0 ? widget.fmtQty(widget.entry.requiredQty) : '',
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(14),
        border: Border.all(
          color: hasError ? _C.danger : isValid ? _C.success : _C.border,
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            Container(
              ...
              child: Text(e.item.emoji, style: const TextStyle(fontSize: 20)),
            ),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(e.item.name, style: const TextStyle(
                  fontSize: 13, fontWeight: FontWeight.w700, color: _C.textPri)),
              Text('Stock: $stockStr $baseUnit  •  ₹${e.item.costPerUnit}/$baseUnit',
            ...
            GestureDetector(
              ...
              child: Container(
                padding: const EdgeInsets.all(6),
                decoration: BoxDecoration(color: _C.dangerBg,
                    borderRadius: BorderRadius.circular(8)),
                child: const Icon(Icons.close_rounded, size: 14, color: _C.danger),
              ),
            ),
          ]),
        ),
        ...
        Padding(
          ...
          child: Row(children: [
            const Text('Qty:', style: TextStyle(fontSize: 12,
                fontWeight: FontWeight.w600, color: _C.textSec)),
            const SizedBox(width: 8),
            // Quantity input
            SizedBox(
              width: 90,
              child: TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  filled: true,
                  fillColor: hasError ? _C.dangerBg : isValid ? _C.successBg : _C.bg,
                  contentPadding: const EdgeInsets.symmetric(horizontal: 8, vertical: 10),
                  ...
                  focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8),
                      borderSide: BorderSide(
                          color: hasError ? _C.danger : _C.brand, width: 1.5)),
                ),
              ),
            ),
            const SizedBox(width: 8),
            ...
            Container(
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  ...
                  onChanged: (u) { if (u != null) widget.onUnitChanged(u); },
                ),
              ),
            ),
            const Spacer(),
        ...
        if (hasError)
          Padding(
            ...
            child: Container(
              ...
              child: Row(children: [
                const Icon(Icons.warning_amber_rounded, size: 13, color: _C.danger),
                const SizedBox(width: 6),
                Expanded(child: Text(e.error!, style: const TextStyle(
          ...
          Padding(
            ...
            child: Row(children: [
              const Icon(Icons.check_circle_outline, size: 12, color: _C.success),
              const SizedBox(width: 4),
              Text(
                'After use: ${widget.fmtQty(remaining)} $baseUnit remaining',
                style: const TextStyle(fontSize: 11, color: _C.success, fontWeight: FontWeight.w600),
              ),
            ]),
          ),
        ...
        Padding(
          ...
          child: TextFormField(
            ...
            decoration: InputDecoration(
              ...
              prefixIcon: const Icon(Icons.edit_note_rounded, size: 16, color: _C.textMut),
              filled: true, fillColor: _C.bg,
...
class _RecipeRollupCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final totalCost  = ingredients.fold<double>(
        0, (s, e) => s + (e.item.costPerUnit * e.qtyInBaseUnit));
    ...
    return Container(
      ...
      child: Column(children: [
        Row(children: [
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Recipe Summary', style: TextStyle(
                fontSize: 12, color: Colors.white60, fontWeight: FontWeight.w500)),
            Text('₹${totalCost.toStringAsFixed(2)} estimated cost',
                style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900,
                    color: Colors.white, letterSpacing: -0.5)),
          ])),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text('$validCount / $totalCount', style: const TextStyle(
                fontSize: 18, fontWeight: FontWeight.w900, color: Colors.white)),
            const Text('validated', style: TextStyle(fontSize: 10, color: Colors.white54)),
          ]),
        ]),
        if (totalCount > 0) ...[
          const SizedBox(height: 10),
...
class _IngredientPickerCard extends StatelessWidget {
  ...
  const _IngredientPickerCard({
    required this.loading, required this.inventory, required this.addedIds,
    required this.searchCtrl, required this.onSearchChanged,
    required this.onAdd, required this.onReload,
  });
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        ...
        child: Row(children: [
          Container(
            padding: const EdgeInsets.all(6),
            decoration: BoxDecoration(color: _C.teal, borderRadius: BorderRadius.circular(8)),
            child: const Icon(Icons.inventory_2_outlined, color: Colors.white, size: 15),
          ),
          const SizedBox(width: 10),
          const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Add from Inventory', style: TextStyle(
                fontSize: 13, fontWeight: FontWeight.w700, color: _C.teal)),
            Text('Only items with available stock shown',
          ...
          GestureDetector(
            ...
            child: Container(
              padding: const EdgeInsets.all(6),
              decoration: BoxDecoration(color: _C.teal.withOpacity(0.1),
                  borderRadius: BorderRadius.circular(8)),
              child: const Icon(Icons.refresh_rounded, color: _C.teal, size: 16),
            ),
          ),
        ]),
      ),
        ...
        child: SizedBox(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              hintStyle: const TextStyle(color: _C.textMut, fontSize: 13),
              prefixIcon: const Icon(Icons.search_rounded, size: 18, color: _C.textMut),
              suffixIcon: searchCtrl.text.isNotEmpty
                  ? GestureDetector(
                      onTap: () { searchCtrl.clear(); onSearchChanged(''); },
                      child: const Icon(Icons.close_rounded, size: 16, color: _C.textMut))
                  : null,
              filled: true, fillColor: _C.bg,
      ...
      if (loading)
        ...
        Padding(padding: const EdgeInsets.all(24), child: Column(children: [
          ...
          Text(
            ...
            style: const TextStyle(fontSize: 13, color: _C.textMut), textAlign: TextAlign.center,
          ),
        ]))
      else
        ConstrainedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (_, i) {
              ...
              return _InventoryPickerRow(
                item: item, isAdded: addedIds.contains(item.id),
                onAdd: () => onAdd(item),
              );
            },
          ),
...
class _InventoryPickerRow extends StatelessWidget {
  ...
  const _InventoryPickerRow({required this.item, required this.isAdded, required this.onAdd});

  Color get _statusColor {
    switch (item.status) {
      case StockStatus.inStock:   return _C.success;
      case StockStatus.lowStock:  return _C.warning;
      case StockStatus.critical:  return _C.danger;
      default:                    return _C.textMut;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: isAdded ? _C.teal : _C.border,
            width: isAdded ? 1.5 : 1),
      ),
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            const SizedBox(width: 4),
            Text('$stockStr $baseUnit', style: TextStyle(
                fontSize: 11, color: _statusColor, fontWeight: FontWeight.w600)),
            const SizedBox(width: 6),
            Text('· ${item.category}  · ₹${item.costPerUnit}/$baseUnit',
          ...
          if (units.length > 1)
            Padding(
              ...
              child: Row(children: [
                const Text('Units: ', style: TextStyle(fontSize: 10, color: _C.textMut)),
                ...units.map((u) => Container(
                  ...
                  child: Text(u, style: const TextStyle(
                      fontSize: 9, color: _C.teal, fontWeight: FontWeight.w700)),
                )),
              ]),
            ),
        ])),
        AnimatedSwitcher(
          ...
          child: isAdded
              ? Container(key: const ValueKey('added'),
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                  decoration: BoxDecoration(color: _C.teal, borderRadius: BorderRadius.circular(8)),
              ...
              : GestureDetector(key: const ValueKey('add'), onTap: onAdd,
                  child: Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                    decoration: BoxDecoration(
                      color: _C.tealLight, borderRadius: BorderRadius.circular(8),
                      border: Border.all(color: _C.teal.withOpacity(0.4)),
                    ),
                    child: const Row(mainAxisSize: MainAxisSize.min, children: [
                      Icon(Icons.add_rounded, color: _C.teal, size: 13),
                      SizedBox(width: 3),
                      Text('Add', style: TextStyle(color: _C.teal,
                          fontSize: 11, fontWeight: FontWeight.w700)),
                    ]),
                  )),
        ),
...
class _RecipeToggleBannerWidget extends StatelessWidget {
  ...
  const _RecipeToggleBannerWidget({required this.enabled, required this.onToggle});
  ...
  Widget build(BuildContext context) => AnimatedContainer(
    ...
    decoration: BoxDecoration(
      ...
      borderRadius: BorderRadius.circular(16),
      border: Border.all(color: enabled ? _C.teal : _C.border, width: enabled ? 0 : 1),
      boxShadow: enabled ? [BoxShadow(
          color: _C.teal.withOpacity(0.25), blurRadius: 16, offset: const Offset(0, 6))] : [],
    ),
    child: Row(children: [
      Container(
        padding: const EdgeInsets.all(10),
        ...
        child: Text(enabled ? '🧪' : '🍳', style: const TextStyle(fontSize: 22)),
      ),
      const SizedBox(width: 14),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(enabled ? 'Recipe Builder Active' : 'Add a Recipe?',
            style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800,
      ...
      Switch(value: enabled, onChanged: onToggle,
          ...
          inactiveThumbColor: _C.textMut, inactiveTrackColor: _C.border),
    ]),
  );
}
...
class _ImagePickerWidget extends StatelessWidget {
  ...
  final VoidCallback onTap;
  const _ImagePickerWidget({this.imageFile, this.existingUrl, required this.onTap});
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: has ? _C.brand.withOpacity(0.4) : _C.border,
              width: has ? 1.5 : 1),
        ),
        child: has
            ...
            : Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                const SizedBox(height: 6),
                Text('Add item photo', style: TextStyle(fontSize: 13, color: Colors.grey.shade500)),
                Text('JPEG / PNG up to 5 MB', style: TextStyle(fontSize: 10, color: Colors.grey.shade400)),
              ]),
      ),
    );
  }
...
class _VegToggleWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    _VegOpt(label: 'Vegetarian', dot: const Color(0xFF16A34A),
        selected: isVeg, onTap: () => onChanged(true)),
    const SizedBox(width: 10),
    _VegOpt(label: 'Non-Veg', dot: const Color(0xFFB91C1C),
        selected: !isVeg, onTap: () => onChanged(false)),
  ]);
}

class _VegOpt extends StatelessWidget {
  final String label; final Color dot; final bool selected; final VoidCallback onTap;
  const _VegOpt({required this.label, required this.dot, required this.selected, required this.onTap});
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: selected ? dot : _C.border, width: selected ? 1.5 : 1),
      ),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
    ...
    child: Row(children: [
      ...
      Expanded(child: SliderTheme(
        data: SliderTheme.of(context).copyWith(
          activeTrackColor: Colors.amber, inactiveTrackColor: _C.border,
          thumbColor: Colors.amber, overlayColor: Colors.amber.withOpacity(0.15),
          thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 8), trackHeight: 3,
        ),
        child: Slider(value: value, min: 1, max: 5, divisions: 8, onChanged: onChanged),
      )),
      Container(
        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
        decoration: BoxDecoration(color: Colors.amber.withOpacity(0.12),
            borderRadius: BorderRadius.circular(8)),
        child: Text(value.toStringAsFixed(1), style: const TextStyle(
            fontWeight: FontWeight.w800, fontSize: 13, color: Colors.amber)),
      ),
    ]),
  );
}
...
class _FlagChip extends StatelessWidget {
  final String emoji, label; final bool active;
  final Color activeColor; final VoidCallback onTap;
  const _FlagChip({required this.emoji, required this.label,
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: active ? activeColor : _C.border, width: active ? 1.5 : 1),
      ),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
...
class _InfoBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    padding: const EdgeInsets.all(16),
    decoration: BoxDecoration(color: _C.surfaceAlt,
...
class _ErrorBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
    decoration: BoxDecoration(color: _C.dangerBg, borderRadius: BorderRadius.circular(10),
        border: Border.all(color: _C.danger.withOpacity(0.3))),
    child: Row(children: [
      const Icon(Icons.error_outline, color: _C.danger, size: 16),
      const SizedBox(width: 8),
      Expanded(child: Text(msg, style: const TextStyle(fontSize: 12, color: _C.danger),
...
class _PrimaryButton extends StatelessWidget {
  ...
  const _PrimaryButton({required this.label, required this.icon,
      required this.color, this.onTap, this.isLoading = false});
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: onTap != null ? [BoxShadow(
            color: color.withOpacity(0.35), blurRadius: 16, offset: const Offset(0, 5))] : [],
      ),
      child: isLoading
...
class _OutlineButton extends StatelessWidget {
  ...
  const _OutlineButton({required this.label, this.onTap, this.isLoading = false});
    ...
    child: Container(
      ...
      child: isLoading
          ...
          : Text(label, style: const TextStyle(
              color: _C.textSec, fontSize: 14, fontWeight: FontWeight.w700)),
    ),
  );
}