...
class DashShimmer extends StatefulWidget {
  final double width;
  final double height;
  final double radius;
  ...
  @override
  State<DashShimmer> createState() => _DashShimmerState();
}

class _DashShimmerState extends State<DashShimmer>
...
// ─────────────────────────────────────────────────────────────────────────────
//  FULL-SCREEN DASHBOARD SKELETON
//  Mirrors the exact structure of the real dashboard so there is no layout
//  jump when data arrives.  Every section has a corresponding shimmer block.
// ─────────────────────────────────────────────────────────────────────────────
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  DashShimmer(width: 44, height: 44, radius: 14),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        DashShimmer(width: 160, height: 16, radius: 8),
                        const SizedBox(height: 5),
                        DashShimmer(width: 100, height: 11, radius: 6),
                      ],
                    ),
                  ),
                  DashShimmer(width: 60, height: 26, radius: 10),
                ],
              ),
            ),
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    // Section label
                    DashShimmer(width: 80, height: 10, radius: 5),
                    ...
                    const SizedBox(height: 12),
                    // KPI wide card
                    DashShimmer(width: double.infinity, height: 58, radius: 16),
                    ...
                    const SizedBox(height: 24),
                    // Top items skeleton
                    DashShimmer(width: 100, height: 10, radius: 5),
                    const SizedBox(height: 12),
                    DashShimmer(
                      width: double.infinity,
                      height: 260,
                    ...
                    const SizedBox(height: 24),
                    // Staff performance skeleton
                    DashShimmer(width: 120, height: 10, radius: 5),
                    const SizedBox(height: 12),
                    DashShimmer(
                      width: double.infinity,
                      height: 220,
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  void initState() {
    super.initState();
    // Kick off data fetching after the first frame.
    // The skeleton is already visible at this point.
  ...
  Widget build(BuildContext context) {
    return Consumer<DashboardProvider>(
      builder: (context, prov, _) {
        // ── SKELETON-FIRST ──────────────────────────────────────────────────
        // isReady is false until the very first successful fetch completes.
        // During that time the full skeleton is shown — no partial UI, no spinner.
        if (!prov.isReady) return const _DashboardSkeleton();

        // ── Error state (only after first load has been attempted) ──────────
        if (prov.error != null && !prov.isLoading) {
          return _buildErrorScaffold(prov);
        }
                ...
                Expanded(
                  child: RefreshIndicator(
                    ...
                    child: ListView(
                      padding: const EdgeInsets.fromLTRB(16, 8, 16, 32),
                      children: [
                        // While a period re-fetch is running, show KPI skeletons
                        // instead of stale numbers — the other sections keep showing.
                        prov.isLoading
                            ? _buildKPICardsSkeleton()
                            : _buildKPICards(prov),
                        const SizedBox(height: 20),
                        // Analytics section — has its own internal skeleton for
                        // period switches; never shows raw spinner.
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          // Report button — only for admin/manager/owner
          if (prov.isAdminLevel) ...[
            GestureDetector(
  ...
  // ── KPI skeleton (period re-fetch only, never initial load) ─────────────────
  ...
  // ── Error scaffold ───────────────────────────────────────────────────────────
  Widget _buildErrorScaffold(DashboardProvider prov) {
    return Scaffold(
      backgroundColor: _C.bg,
      body: Center(
        child: Padding(
...
/*completed allimport 'package:flutter/material.dart';
import 'package:pos_app/screens/revenue_analytics_screen.dart';
import 'package:provider/provider.dart';
import '../../providers/dashboard_provider.dart';
import '../../providers/analytics_provider.dart';
