// lib/providers/tables_provider.dart
import 'dart:async';
...
class TablesProvider extends ChangeNotifier {
  ...
  // ── Fetch calendar reservations (all upcoming, not just today) ────────────
  ...
  Future<void> _fetchTables() async {
    ...
    try {
      ...
      _tables
        ..clear()
        ..addAll(
          (rows as List).map((r) => _rowToTable(r as Map<String, dynamic>)),
        );
      notifyListeners();
      _runNotifCheck();
    } catch (e) {
    ...
    if (row['reservation_id'] != null) {
      ...
      if (isToday && (resStatus == 'active' || resStatus == 'seated')) {
        reservation = Reservation(
          ...
          // check_in: set only on actual arrival — may be null
          checkIn: row['res_check_in'] != null
              ? parseToIST(row['res_check_in'] as String)
              : null,
          // check_out: planned end time — may be null for open-ended bookings
  ...
  Future<void> addTable(RestaurantTable t) async {
    ...
    try {
      await _sb
          .from(_kTables)
          .insert(_tableToRow(t, isCreate: true)..remove('id'));
      await _refreshAll();
    } catch (e) {
  ...
  /// Seat guests — uses fn_seat_guest RPC which records ACTUAL check-in time.
  Future<void> seatGuests(String tableId, String customerName) async {
    try {
      ...
      // ✅ RPC writes: check_in = NOW() (actual arrival), session_id = new UUID
      await _sb.rpc(
        'fn_seat_guest',
        params: {
          'p_table_id': tableId,
  ...
  /// Clear table — uses fn_complete_reservation RPC which records ACTUAL check-out.
  Future<void> clearTable(String tableId) async {
    try {
      final t = _tables.where((t) => t.id == tableId).firstOrNull;
      // ✅ Cancel ALL future notifications before clearing
      if (t?.reservation != null) {
        _notif.cancelReservationScheduled(t!.reservation!.id, t.tableNumber);
        _notif.clearReservationKeys(t.reservation!.id);
      }
      // Clear long-seated keys for this table
      _notif.clearReservationKeys('ls_${tableId}');

      // ✅ RPC writes: check_out = NOW() (actual departure), session_id = NULL
      await _sb.rpc(
        'fn_complete_reservation',
        params: {
          'p_table_id': tableId,
          'p_actual_checkout': DateTime.now().toUtc().toIso8601String(),
          'p_staff_uid': _userCtx?.uid,
          'p_staff_name': _userCtx?.name,
        },
      );
  ...
  /// Add reservation — stores planned times ONLY.
  /// check_in is intentionally NOT set here; it's set when guest physically arrives.
  Future<void> addReservation(String tableId, Reservation res) async {
    try {
      ...
      await _sb.from(_kReservations).insert({
        ...
        // ✅ check_out = planned end time (used for slot availability check)
        'check_out': res.checkOut?.toUtc().toIso8601String(),
        // ✅ check_in intentionally omitted — set only on actual arrival via fn_seat_guest
        'notes': res.notes,
        'status': 'active',
        'business_id': ctx.businessId,
        'business_name': ctx.businessName,
    ...
    try {
      await _sb
          ...
          .update({
            ...
            // ✅ Never update check_in here — only set by fn_seat_guest
            'notes': updated.notes,
            'updated_by_uid': _userCtx?.uid,
            'updated_by_name': _userCtx?.name,
          })
