...
class InventoryProvider extends ChangeNotifier {
  ...
  Future<bool> addItem(InventoryItem item) async {
    ...
    try {
      ...
      // FIX: use named parameters to match InventoryRepository.addItem signature
  ...
  Future<bool> updateItem(InventoryItem updated) async {
    ...
    try {
      ...
      // FIX: use named parameters to match InventoryRepository.updateItem signature
  ...
  Future<void> recordTransaction({
    ...
    try {
      ...
      // FIX: pass all required named parameters; rename itemUnit → unit
...
class InventoryProvider extends ChangeNotifier {
  ...
  double get totalInventoryValue =>
      _items.fold(0, (s, i) => s + i.totalValue);
  ...
  ///
  /// Before inserting, the supplier name is resolved through
  /// [OtherSupplierService]: if no UUID supplier is linked yet, a new
  /// supplier row is created (or an existing one reused) so the item always
  /// carries a valid foreign-key reference.
  Future<bool> addItem(InventoryItem item) async {
    ...
    try {
      // ── Resolve supplier → always get a valid UUID or null ──────────────
      final resolvedSupplierId =
          await OtherSupplierService.instance.resolveOrCreate(
        businessId: _businessId,
        supplierName: item.supplier,
        existingId: item.supplierId,
      );

      // Build a clean item with the resolved supplier id.
      final itemToInsert = resolvedSupplierId != null
          ? item.copyWith(supplierId: resolvedSupplierId)
          : item;
      ...
      // Delegate insert and initial transaction logging to the repository
      await InventoryRepository.instance.addItem(
        itemToInsert,
        _businessId,
        _userUid,
  ...
  ///
  /// Same supplier-resolution logic as [addItem] so that editing an item
  /// that previously had no supplier (or had a sentinel value) will also
  /// create/link the correct supplier record.
  Future<bool> updateItem(InventoryItem updated) async {
    if (!canManageStock) {
      _errorMessage = 'You do not have permission to manage stock.';
      notifyListeners();
    ...
    try {
      ...
      final data = itemToUpdate.toJson(_businessId);
      await InventoryRepository.instance.updateItem(
        itemToUpdate,
        _businessId,
        data,
  ...
  Future<void> recordTransaction({
    ...
    try {
      ...
      final newStock =
          (item.currentStock + delta).clamp(0.0, item.maxCapacity);
  ...
  // _insertTransaction was moved to InventoryRepository
  String generateId() => 'i${DateTime.now().millisecondsSinceEpoch % 10000}';
