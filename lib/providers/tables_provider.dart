...
// NEW IMPORT — add this file to your project
...
class TablesProvider extends ChangeNotifier {
  ...
  final _notifService = ReservationNotificationService();
  ...
  /// Returns tables that have a reservation on the given date.
  ...
  // ── Notification timer — every 1 minute ──────────────────
  ...
  void _runNotifCheck() {
    _notifService.checkAll(
      tables: _tables,
      businessName: _userCtx?.businessName ?? '',
      longSeatedMinutes: 120, // configurable
    );
  }
  ...
  Future<void> _fetchTables() async {
    ...
    try {
      ...
      notifyListeners();
      // Trigger notification check after fresh data
      _runNotifCheck();
    } catch (e) {
  ...
  void cancelReservation(String tableId) {
    // Clear notification keys for this table's reservation
    final table = _tables.firstWhere(
      (t) => t.id == tableId,
      orElse: () => _tables.first,
    );
    if (table.reservation != null) {
      _notifService.clearReservationKeys(table.reservation!.id);
    }
  ...
  Future<void> loadHistory({
    ...
    try {
      ...
      final toDate =
          _historyTo ?? DateTime.now().add(const Duration(days: 1));
      ...
      final items = (rows as List)
          .map(
            (r) =>
                ReservationHistoryItem.fromMap(r as Map<String, dynamic>),
          )
          .toList();
  ...
  List<RestaurantTable> get todayReservations {
    return reservationsForDate(DateTime.now());
  }
  ...
  /// Tables where guests have been seated longer than [minutes]
  List<RestaurantTable> get longSeatedTables {
    return longOccupiedTables(120);
  }
  ...
  Map<TableSection, Map<String, int>> get sectionStats {
    ...
    for (final s in TableSection.values) {
      ...
      result[s] = {
        ...
        'available':
            sectionTables.where((t) => t.status == TableStatus.available).length,
        'occupied':
            sectionTables.where((t) => t.status == TableStatus.occupied).length,
        'reserved':
            sectionTables.where((t) => t.status == TableStatus.reserved).length,
        'cleaning':
            sectionTables.where((t) => t.status == TableStatus.cleaning).length,
