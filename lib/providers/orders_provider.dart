...
// FIX: All employees now see ALL orders for their business.
// Removed staffUid filter from fetchOrders — every logged-in user
// sees the full company order list, regardless of role.
// Realtime subscription also updated to accept all business orders.
...
class OrdersProvider extends ChangeNotifier {
  ...
  Future<void> _loadUserFromFirestore() async {
    try {
      ...
      if (firebaseUser == null) {
        debugPrint('📦 _loadUserFromFirestore: No Firebase user');
        return;
      }
      ...
      debugPrint('📦 user loaded: name=$_name role=$_role biz=$_businessId');
  ...
  // ══════════════════════════════════════════════════════════════════════════
  //  FETCH ORDERS
  //  ✅ FIX: staffUid is NEVER passed — all employees see ALL company orders
  // ══════════════════════════════════════════════════════════════════════════
  ...
  Future<void> fetchOrders() async {
    ...
    // Ensure uid/role are loaded (needed for create/update operations)
    if (_uid.isEmpty) {
      await _loadUserFromFirestore();
      if (_uid.isEmpty) {
        final stored = await StorageService.instance.getUserData();
    ...
    try {
      debugPrint(
        '📦 fetchOrders: fetching ALL orders for biz=$_businessId (role=$_role)',
      );

      // ✅ No staffUid filter — fetch all orders for the business
      _orders = await OrdersService.instance.fetchTodayOrders(
        businessId: _businessId,
        staffUid: null, // ← always null: show everyone's orders
      );
  ...
  // ══════════════════════════════════════════════════════════════════════════
  //  REALTIME
  //  ✅ FIX: No role-based filter — all business orders are accepted
  // ══════════════════════════════════════════════════════════════════════════
  ...
  void _subscribeRealtime() {
    ...
    _ordersChannel = OrdersService.instance.subscribeToOrders(
      ...
      onEvent: (order, eventType) async {
        ...
        // ✅ FIX: Accept ALL orders for this business — no uid filter
        if (isNew) {
          _orders.insert(0, order);
        } else {
          _orders[idx] = order;
        ...
        // Notify only for orders created by OTHER staff members
...
class OrdersProvider extends ChangeNotifier {
  ...
  Future<void> fetchOrders() async {
    ...
    try {
      ...
      _orders = await OrdersService.instance.fetchTodayOrders(
        businessId: _businessId,
        //command for (entire order for the users) Only fetch all orders if truly admin level AND uid is confirmed
